...
class DatabaseEngine {
  ...
  public async syncFromPostgres() {
    ...
    try {
      const [uRes, cRes, pRes, smRes, oRes, oiRes, chRes, chiRes, ijRes, wRes, srRes] = await Promise.all([
        this.pgPool.query('SELECT id, name, email, password_hash as "passwordHash", role, phone, avatar_url as "avatarUrl", created_at as "createdAt", updated_at as "updatedAt" FROM users'),
        this.pgPool.query('SELECT id, user_id as "userId", name, business_name as "businessName", email, phone, gst_number as "gstNumber", customer_type as "customerType", status, follow_up_date as "followUpDate", notes, created_at as "createdAt", updated_at as "updatedAt" FROM customers'),
        this.pgPool.query('SELECT id, sku, name, category_id as "categoryId", category_name as "categoryName", unit_price::float as "unitPrice", current_stock as "currentStock", min_stock_alert as "minStockAlert", location, installation_eligible as "installationEligible", warranty_months as "warrantyMonths", description, specifications, created_at as "createdAt", updated_at as "updatedAt" FROM products'),
        this.pgPool.query('SELECT id, product_id as "productId", product_name as "productName", product_sku as "productSku", quantity, movement_type as "movementType", reason, reference_type as "referenceType", reference_id as "referenceId", created_by as "createdBy", created_by_name as "createdByName", created_at as "createdAt" FROM stock_movements'),
        this.pgPool.query('SELECT id, order_number as "orderNumber", customer_id as "customerId", customer_name as "customerName", address_id as "addressId", address_text as "addressText", installation_type as "installationType", total_amount::float as "totalAmount", status, created_at as "createdAt", updated_at as "updatedAt" FROM orders'),
        this.pgPool.query('SELECT id, order_id as "orderId", product_id as "productId", product_name as "productName", sku, unit_price::float as "unitPrice", quantity, installation_eligible as "installationEligible" FROM order_items'),
        this.pgPool.query('SELECT id, challan_number as "challanNumber", order_id as "orderId", customer_id as "customerId", customer_name as "customerName", status, total_quantity as "totalQuantity", created_by as "createdBy", created_by_name as "createdByName", confirmed_at as "confirmedAt", created_at as "createdAt", updated_at as "updatedAt" FROM challans'),
        this.pgPool.query('SELECT id, challan_id as "challanId", product_id as "productId", product_name as "productName", sku, unit_price::float as "unitPrice", quantity FROM challan_items'),
        this.pgPool.query('SELECT id, job_number as "jobNumber", order_id as "orderId", customer_id as "customerId", customer_name as "customerName", customer_phone as "customerPhone", address_id as "addressId", address_text as "addressText", latitude::float, longitude::float, scheduled_date as "scheduledDate", time_slot as "timeSlot", required_crew_size as "requiredCrewSize", status, checklist_state as "checklistState", notes, created_at as "createdAt", updated_at as "updatedAt" FROM installation_jobs'),
        this.pgPool.query('SELECT id, customer_id as "customerId", order_id as "orderId", product_id as "productId", product_name as "productName", serial_number as "serialNumber", start_date as "startDate", end_date as "endDate", status FROM warranties'),
        this.pgPool.query('SELECT id, service_number as "serviceNumber", customer_id as "customerId", customer_name as "customerName", customer_phone as "customerPhone", address_id as "addressId", address_text as "addressText", latitude::float, longitude::float, product_id as "productId", product_name as "productName", problem_category as "problemCategory", description, warranty_status as "warrantyStatus", scheduled_date as "scheduledDate", time_slot as "timeSlot", status, resolution_notes as "resolutionNotes", total_cost::float as "totalCost", created_at as "createdAt", updated_at as "updatedAt" FROM service_requests'),
      ]);
